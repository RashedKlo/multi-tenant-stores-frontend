...
export function CartItemCard({
	...
	return (
		<article className={["flex gap-3 rounded-2xl border border-border bg-card p-3 shadow-sm transition-all sm:gap-4 sm:p-4", pending ? "pointer-events-none opacity-60" : "hover:shadow-md"].join(" ")}>
			<Link href={productLink} className="relative h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-muted sm:h-24 sm:w-24" tabIndex={-1} aria-hidden="true">
				{item.productImage ? <Image src={item.productImage} alt={item.productName} fill sizes="(max-width: 640px) 80px, 96px" className="object-cover" unoptimized /> : <span className="flex h-full w-full items-center justify-center text-lg font-bold text-muted-foreground">{item.productName?.slice(0, 1) || "P"}</span>}
			</Link>
			<div className="flex min-w-0 flex-1 flex-col gap-1.5">
				...
				<div className="mt-auto flex items-center justify-between pt-1">
					<div role="group" aria-label={t("quantityLabel")} className="flex items-center rounded-full border border-border">
						<StepperButton label={t("decrease")} disabled={pending || item.quantity <= 1} onClick={() => onQuantityChange(item.quantity - 1)}><MinusIcon /></StepperButton>
						<output className="min-w-7 select-none text-center text-sm font-semibold tabular-nums" dir="ltr">{item.quantity}</output>
						<StepperButton label={t("increase")} disabled={pending} onClick={() => onQuantityChange(item.quantity + 1)}><PlusIcon /></StepperButton>
					</div>
					<p className="text-sm font-bold tabular-nums" dir="ltr">{formatPrice(item.itemTotalPrice, locale)}</p>
				</div>
			</div>
		</article>
	);
...
function formatPrice(amount: number, locale: string) {
	return new Intl.NumberFormat(locale, { style: "currency", currency: "SYP", maximumFractionDigits: 0 }).format(amount);
}
...
function StepperButton({ label, disabled, onClick, children }: { label: string; disabled?: boolean; onClick: () => void; children: React.ReactNode }) {
	return <button type="button" onClick={onClick} disabled={disabled} aria-label={label} className="grid h-9 w-9 place-items-center rounded-full text-muted-foreground transition-colors hover:bg-muted hover:text-foreground disabled:pointer-events-none disabled:opacity-40">{children}</button>;
}